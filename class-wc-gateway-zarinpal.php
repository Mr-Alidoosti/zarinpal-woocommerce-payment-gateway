    ...
    if (!function_exists('Woocommerce_Add_ZarinPal_Gateway') && class_exists('WC_Payment_Gateway') && !class_exists('WC_ZPal')) {
        ...
        class WC_ZPal extends WC_Payment_Gateway
        {
            ...
            public function __construct()
            {
                ...
                $this->st_order_status = $this->settings['st_order_status'];
            ...
            public function init_form_fields()
            {
                $this->form_fields = apply_filters('WC_ZPal_Config', array(
                        ...
                        'st_order_status' => array(
                            'title' => __('وضعیت سفارش بعد از پرداخت موفق'),
                            'type' => 'select',
                            'options'     =>array(
                                'pending'    => __( 'درحال بررسی'),
                                'processing' => __( 'درحال پردازش'),
                                'on-hold'    => __( 'نگه داشته شده'),
                                'completed'  => __( 'تکمیل شده'),
                                'cancelled'  => __( 'لغو شده'),
                                'refunded'   => __( 'بازگشت خورده' ),
                                'failed'     => __( 'ناموفق'),
                            ),
                            'description' => __('وضعیت سفارش بعد از پرداخت موفق و بازگشت از درگاه بر روی این حالت انتخابی تنظیم می شود.'),
                            'default' => 'completed',
                        ),
                    )
                );
                ...
                if ($order_id) {
                    ...
                    if ($order->status !== 'completed') {
                        ...
                        if ($Status === 'completed' && isset($Transaction_ID) && $Transaction_ID !== 0) {
                            ...
                            $order->update_status($this->st_order_status);
